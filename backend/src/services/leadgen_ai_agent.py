...
from src.services import GooglePlacesService, YelpFusionService, DiscoveryService
...
class LeadGenAIAgent:
    ...
    async def process_message(
        self, user_message: str, session_id: str, message_history: List[Dict[str, str]]
    ) -> Dict[str, Any]:
        """
    ...
    def _update_context(
        self, session_id: str, user_message: str, message_history: List[Dict[str, str]]
    ):
    ...
    def _analyze_intent(self, user_message: str) -> Dict[str, Any]:
        ...
        # Check for demo generation intent
            ...
            word in message_lower
            for word in ["demo", "create", "build", "website", "template"]
        ):
            return {
        ...
        # Check for data export intent
            ...
            word in message_lower
            for word in ["export", "download", "save", "data", "report"]
        ):
            return {
                "type": "data_export",
                "confidence": 0.7,
        ...
        # Check for outreach generation intent
            ...
            word in message_lower
            for word in ["outreach", "email", "message", "contact", "follow up"]
        ):
            return {
        ...
        # Check for parameter confirmation
            ...
            word in message_lower
            for word in ["yes", "no", "confirm", "correct", "right", "wrong"]
        ):
            return {
        ...
        # Check for general help
            ...
            word in message_lower
            for word in ["help", "what can you do", "how does this work", "explain"]
        ):
            return {"type": "general_help", "confidence": 0.9, "parameters": {}}
    ...
    async def _handle_business_discovery(
        self, intent: Dict[str, Any], session_id: str
    ) -> Dict[str, Any]:
        ...
        discovery_result = await self.discovery_service.discover_businesses(
            location=location,
            niche=niche,
            max_results=parameters.get("max_businesses", 10),
        )
            ...
            "tool_calls": [
                {
                    "function": {
                        ...
                        "arguments": json.dumps(discovery_result),
                    }
                }
            ],
            "requires_confirmation": False,
            ...
            "tool_calls": [
                {
                    "function": {
                        "name": "score_websites",
                        "arguments": json.dumps({"scoring_method": "comprehensive"}),
                    }
                }
            ],
            "requires_confirmation": False,
            ...
            "tool_calls": [
                {
                    "function": {
                        ...
                        "arguments": json.dumps({"template_style": "modern"}),
                    }
                }
            ],
            "requires_confirmation": False,
            ...
            "tool_calls": [
                {
                    "function": {
                        "name": "export_data",
                        "arguments": json.dumps({"format": "csv"}),
                    }
                }
            ],
            "requires_confirmation": False,
            ...
            "tool_calls": [
                {
                    "function": {
                        ...
                        "arguments": json.dumps({"outreach_type": "email"}),
                    }
                }
            ],
            "requires_confirmation": False,
    ...
    async def _handle_parameter_confirmation(
        self, intent: Dict[str, Any], session_id: str
    ) -> Dict[str, Any]:
...
What would you like me to help you with?
        ...
        return {"content": help_text, "tool_calls": [], "requires_confirmation": False}
    ...
    def update_context_from_tool_result(
        self, session_id: str, tool_name: str, tool_result: Dict[str, Any]
    ):
        ...
        context["tool_results"].append(
            {"tool_name": tool_name, "result": tool_result, "timestamp": datetime.now()}
        )
        ...
        if tool_result.get("success") and tool_result.get("result"):
            ...
            if "businesses" in result_data:
                context["extracted_parameters"]["business_count"] = len(
                    result_data["businesses"]
                )
    ...
    async def discover_businesses(
        ...
        """
        return await self.discovery_service.discover_businesses(
            location, niche, max_results, run_id
        )
    ...
    def _process_and_combine_results(
        ...
        for result in results:
            if result and not result.get("error"):
                # Normalize and add businesses to the combined list
                # This part needs to be adapted based on the actual structure of the response models
                # For now, we'll just append them
                # In a real implementation, you would normalize the data structure
                # and use a unique identifier (like name + address) to avoid duplicates
                pass  # Implement normalization and merging logic here

        logger.info(
            f"🔍 AI Agent combined and processed {len(combined)} unique businesses."
        )
        return combined
