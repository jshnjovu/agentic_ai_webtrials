...
class FallbackScoringService(BaseService):
    ...
    def run_fallback_scoring(
        ...
        try:
            ...
            return self._create_error_response(
                str(e), "fallback_scoring", website_url, business_id, run_id
            )
    ...
    def _analyze_failure(self, failure_reason: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pattern = self.failure_patterns.get(
                failure_type, self.failure_patterns["UNKNOWN_ERROR"]
            )
    ...
    def _execute_retry_logic(
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.log_error(
                    e, f"retry_attempt_{retry_attempts}", run_id, business_id
                )
                continue
    ...
    def _attempt_lighthouse_recovery(
        self, website_url: str, business_id: str, run_id: Optional[str]
    ) -> bool:
    ...
    def _calculate_reliability_score(
        self, severity: FailureSeverity, data_completeness: float
    ) -> float:
        ...
        try:
            ...
            reliability_score = (
                base_score
                - severity_penalties.get(severity, 25.0)
    ...
    def _calculate_confidence_adjustment(
        ...
        try:
            ...
            adjustment = (
                base_adjustment
                * severity_adjustments.get(severity, 0.7)
                * completeness_factor
            )
    ...
    def _determine_quality_indicators(
        ...
        try:
            ...
            indicators["scores_reasonable"] = (
                0 <= fallback_score.overall_score <= 100
                and all(
                    0 <= score <= 100
                    for score in [
            ...
            indicators["failure_analysis_complete"] = all(
                key in failure_analysis
                for key in ["severity", "decision", "retry_count"]
            )

            # Check if confidence level is appropriate
            indicators["confidence_appropriate"] = (
                fallback_score.confidence_level == ConfidenceLevel.LOW
            )
    ...
    def _generate_quality_recommendation(
        ...
        try:
            ...
            if data_completeness < 50:
                recommendation += (
                    " Limited data availability may affect result accuracy."
                )

            # Add severity note
            if failure_analysis["severity"] in [
                FailureSeverity.HIGH,
                FailureSeverity.CRITICAL,
            ]:
                recommendation += (
                    " High severity failure suggests potential systemic issues."
                )
