...
class LeadGenContextManager:
    ...
    def advance_workflow_step(self, session_id: str, new_step: WorkflowStep):
        ...
        logger.info(
            f"🔄 Workflow step advanced for session {session_id}: {old_step.value} → {new_step.value}"
        )
    ...
    def add_message_to_history(
        ...
        message = MessageHistory(
            timestamp=datetime.now(), role=role, content=content, metadata=metadata
        )
    ...
    def set_pending_confirmation(
        ...
        if session_id not in self.workflow_states:
            logger.warning(
                f"⚠️ Attempted to set confirmation for non-existent session {session_id}"
            )
            return
        ...
        logger.info(
            f"⏳ Set pending confirmation for session {session_id}: {confirmation_type.value}"
        )
    ...
    def resolve_confirmation(
        ...
        if session_id not in self.workflow_states:
            logger.warning(
                f"⚠️ Attempted to resolve confirmation for non-existent session {session_id}"
            )
            return None
        ...
        logger.info(
            f"✅ Resolved confirmation for session {session_id}: confirmed={confirmed}"
        )
    ...
    def handle_workflow_error(
        self, session_id: str, error_message: str, recoverable: bool = True
    ):
        """Handle workflow errors and store them in the session state"""

        if session_id not in self.workflow_states:
            logger.warning(
                f"⚠️ Attempted to handle error for non-existent session {session_id}"
            )
            return
        ...
        logger.error(
            f"❌ Workflow error for session {session_id}: {error_message} (recoverable: {recoverable})"
        )
    ...
    def add_warning(self, session_id: str, warning_message: str):
        ...
        if session_id not in self.workflow_states:
            logger.warning(
                f"⚠️ Attempted to add warning for non-existent session {session_id}"
            )
            return
    ...
    def get_workflow_progress(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "confirmation_type": (
                workflow_state.confirmation_type.value
                if workflow_state.confirmation_type
                else None
            ),
