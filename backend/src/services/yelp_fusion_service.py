...
class YelpFusionService(BaseService):
    ...
    def search_businesses(self, request: YelpBusinessSearchRequest) -> YelpBusinessSearchResponse | YelpBusinessSearchError:
        ...
        try:
            ...
            # Check rate limiting
            can_request, reason = self.rate_limiter.can_make_request("yelp_fusion", request.run_id)
            if not can_request:
    ...
    def _process_business_results(self, raw_businesses: List[Dict[str, Any]], max_results: int, run_id: Optional[str]) -> List[YelpBusinessData]:
        """
        ...
        try:
            ...
            for i, raw_business in enumerate(raw_businesses[:max_results]):
                try:
                    ...
                    attributes = self._extract_attributes(raw_business.get("attributes", {}))
                    special_hours = self._extract_special_hours(raw_business.get("special_hours", []))
    ...
    def _extract_business_hours(self, raw_hours: List[Dict[str, Any]]) -> List[YelpBusinessHours]:
        ...
        for hour_data in raw_hours:
            try:
                # Validate required fields
                if not isinstance(hour_data.get("day"), int) or not isinstance(hour_data.get("start"), str) or not isinstance(hour_data.get("end"), str):
                    continue
    ...
    def _extract_categories(self, raw_categories: List[Dict[str, Any]]) -> List[YelpBusinessCategory]:
        ...
        for cat_data in raw_categories:
            try:
                # Validate required fields
                if not isinstance(cat_data.get("alias"), str) or not isinstance(cat_data.get("title"), str):
                    continue
    ...
    def _extract_attributes(self, raw_attributes: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for key, value in raw_attributes.items():
            if value is not None and value != "":
                if isinstance(value, dict):
                    ...
                    filtered_nested = {k: v for k, v in value.items() if v is not None and v != ""}
                    if filtered_nested:
    ...
    def _extract_special_hours(self, raw_special_hours: List[Dict[str, Any]]) -> Optional[List[Dict[str, Any]]]:
        ...
        for special_hour in raw_special_hours:
            try:
                ...
                if not isinstance(special_hour.get("date"), str):
                    continue
                if not isinstance(special_hour.get("start"), str) or not isinstance(special_hour.get("end"), str):
                    continue
    ...
    def _extract_social_media(self, raw_business: Dict[str, Any]) -> Optional[Dict[str, str]]:
