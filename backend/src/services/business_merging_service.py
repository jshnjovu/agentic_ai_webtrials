...
class BusinessMergingService(BaseService):
        ...
        try:
            ...
            merged_business = MergedBusinessData(
                ...
                merge_metadata={
                    ...
                    "completeness_scores": {
                        score.source: score.overall_score
                ...
                needs_review=False,  # Placeholder
            )

            # Determine if review is needed
            needs_review = self._determine_if_review_needed(
                merged_business, conflicts_resolved, completeness_scores
            )
            merged_business.needs_review = needs_review
    ...
    def _calculate_completeness_scores(
        ...
        for business in businesses:
            ...
            rating_score = self._calculate_rating_completeness(
    ...
    def _calculate_contact_completeness(
    ...
    def _calculate_rating_completeness(
    ...
    def _merge_business_data(
        ...
        for business in businesses:
            ...
            if business.categories:
                merged_data["categories"] = list(
                    set(merged_data["categories"] + business.categories)
                )
    ...
    def _resolve_field_conflicts(
        ...
        if business.contact_info and merged_data["contact_info"]:
            conflicts.extend(
                self._resolve_contact_conflicts(
    ...
    def _resolve_contact_conflicts(
        self,
        ...
        if (
            ...
            and business_contact.phone
            and primary_contact.phone != business_contact.phone
        ):
        ...
        if (
            ...
            and business_contact.website
            and primary_contact.website != business_contact.website
        ):
            conflict = MergeConflict(
                field_name="website",
                source_values={
        ...
        if (
            ...
            and business_contact.email
            and primary_contact.email != business_contact.email
        ):
            conflict = MergeConflict(
                field_name="email",
                source_values={
    ...
    def _resolve_rating_conflict(
        self, primary_rating: float, business_rating: float, merge_strategy: str
    ) -> MergeConflict:
        ...
        return MergeConflict(
            field_name="rating",
            source_values={"primary": primary_rating, "business": business_rating},
            resolution_strategy=merge_strategy,
    ...
    def _resolve_price_conflict(
        self, primary_price: int, business_price: int, merge_strategy: str
    ) -> MergeConflict:
        """Resolve price level conflicts."""
        # Keep primary price level
        return MergeConflict(
            field_name="price_level",
            source_values={"primary": primary_price, "business": business_price},
            resolution_strategy=merge_strategy,
            resolved_value=primary_price,
            confidence=0.8,
        )

    def _determine_merged_confidence(
        ...
        avg_confidence = sum(
            conflict.confidence for conflict in conflicts_resolved
        ) / len(conflicts_resolved)
    ...
    def _determine_if_review_needed(
        self,
        merged_business: MergedBusinessData,
        ...
        try:
            ...
            review_flag = review_service.flag_uncertain_matches(
                temp_merged, conflicts_resolved, []  # Placeholder for source data
            )
            ...
            return len(conflicts_resolved) > 0 and any(
                c.confidence < 0.7 for c in conflicts_resolved
            )
