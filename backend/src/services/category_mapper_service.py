...
class CategoryMapperService(BaseService):
    ...
    def _initialize_yelp_to_google_mapping(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            # Government & Public Services
            "publicservicesgovt": ["police", "fire_station", "post_office", "city_hall"],
            "police": ["police"],
            ...
            # Other
            "localservices": ["funeral_home", "cemetery", "storage", "moving_company", "plumber", "electrician", "painter", "landscaper"],
    ...
    def map_google_to_yelp(self, google_categories: List[str]) -> List[str]:
        ...
        """
        try:
            self.log_operation(f"Mapping Google categories to Yelp: {google_categories}")
            ...
            for google_cat in google_categories:
                if google_cat in self._google_to_yelp_mapping:
                    ...
                    partial_matches = self._find_partial_matches(google_cat, self._google_to_yelp_mapping.keys())
                    if partial_matches:
                        yelp_categories.extend(self._google_to_yelp_mapping[partial_matches[0]])
                    else:
                        # Add generic category if no match found
                        yelp_categories.append("restaurants" if "food" in google_cat.lower() else "shopping")
        ...
        try:
            ...
            for yelp_cat in yelp_categories:
                if yelp_cat in self._yelp_to_google_mapping:
                    ...
                    # Try to find partial matches
                    partial_matches = self._find_partial_matches(yelp_cat, self._yelp_to_google_mapping.keys())
                    if partial_matches:
                        google_categories.extend(self._yelp_to_google_mapping[partial_matches[0]])
                    else:
                        # Add generic category if no match found
                        google_categories.append("store" if "shop" in yelp_cat.lower() else "food")
    ...
    def _find_partial_matches(self, category: str, available_categories: List[str]) -> List[str]:
        """Find partial matches for a category string."""
        category_lower = category.lower()
        matches = []
        
        for available_cat in available_categories:
            if (category_lower in available_cat.lower() or 
                available_cat.lower() in category_lower):
    ...
    def get_mapping_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_google_mappings = sum(len(categories) for categories in self._google_to_yelp_mapping.values())
            total_yelp_mappings = sum(len(categories) for categories in self._yelp_to_google_mapping.values())
    ...
    def validate_category_mapping(self, source_categories: List[str], target_categories: List[str], 
                                source_type: str = "google") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not expected_target:
                ...
                accuracy = (matches / len(expected_target)) * 100 if expected_target else 0.0
            ...
            return {
                ...
                "validation_score": "good" if accuracy >= 80 else "fair" if accuracy >= 60 else "poor"
            }
