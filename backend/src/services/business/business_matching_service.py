...
class BusinessMatchingService(BaseService):
    ...
    def match_businesses(self, request: BusinessMatchingRequest) -> BusinessMatchingResponse:
        """
        ...
        try:
            if not self.validate_input(request):
                raise ValueError("Invalid input: must have at least 2 businesses to match")
            ...
            self.log_operation("match_businesses_completed", request.run_id, 
                             total_matches=len(matched_groups),
                             total_unmatched=len(unmatched_businesses))
    ...
    def _normalize_businesses(self, businesses: List[BusinessSourceData]) -> List[BusinessSourceData]:
        ...
        for business in businesses:
            ...
            if normalized_business.name:
                normalized_business.name = self._normalize_business_name(normalized_business.name)
    ...
    def _find_matches(self, businesses: List[BusinessSourceData], 
                      threshold: float, name_weight: float, 
                      address_weight: float, coordinate_weight: float) -> Tuple[List[List[BusinessMatchCandidate]], List[BusinessSourceData]]:
        ...
        for i, business in enumerate(businesses):
            ...
            for j, other_business in enumerate(businesses[i+1:], i+1):
                ...
                score = self._calculate_similarity_score(
                    business, other_business, name_weight, address_weight, coordinate_weight
                )
                ...
                for match in matches:
                    candidate = BusinessMatchCandidate(
                        ...
                        match_score=self._calculate_similarity_score(
                            business, match, name_weight, address_weight, coordinate_weight
                        ),
                        is_duplicate=True,
                        needs_review=False
                    )
        ...
        unmatched = [businesses[i] for i in range(len(businesses)) if i not in processed]
        
        return matched_groups, unmatched
    
    def _calculate_similarity_score(self, business1: BusinessSourceData, 
                                  business2: BusinessSourceData, name_weight: float,
                                  address_weight: float, coordinate_weight: float) -> BusinessMatchScore:
        """Calculate similarity score between two businesses."""
        
        # Name similarity
        name_similarity = self._calculate_name_similarity(business1.name, business2.name)
    ...
    def _calculate_address_similarity(self, location1: BusinessLocation, 
                                    location2: BusinessLocation) -> float:
    ...
    def _calculate_coordinate_proximity(self, location1: BusinessLocation, 
                                      location2: BusinessLocation) -> float:
        """Calculate proximity between two coordinate pairs."""
        if not location1.latitude or not location1.longitude or \
           not location2.latitude or not location2.longitude:
            return 0.0
