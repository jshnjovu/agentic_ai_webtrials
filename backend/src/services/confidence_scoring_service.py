...
class ConfidenceScoringService(BaseService):
    ...
    def calculate_data_confidence(self, business_data: BusinessSourceData) -> float:
        ...
        try:
            ...
            self.log_operation(
                "calculate_data_confidence", getattr(business_data, "run_id", None)
            )
            ...
            field_confidences = {
                ...
                "contact_info": self._calculate_contact_confidence(
                    business_data.contact_info
                ),
                "rating": self._calculate_rating_confidence(business_data.rating),
                "categories": self._calculate_categories_confidence(
            ...
            self.logger.debug(
                f"Calculated confidence for {business_data.source}: {overall_confidence:.3f}"
            )
    ...
    def calculate_merged_confidence(
        self,
        ...
        try:
            ...
            conflict_confidence = self._calculate_conflict_resolution_confidence(
                conflicts_resolved
            )

            # Data consistency confidence
            consistency_confidence = self._calculate_data_consistency_confidence(
                completeness_scores
            )

            # Weighted combination
            weights = {"source": 0.5, "conflict": 0.3, "consistency": 0.2}
    ...
    def add_confidence_indicators(
        self, merged_business: MergedBusinessData
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.log_operation(
                "add_confidence_indicators", getattr(merged_business, "run_id", None)
            )
            ...
            self.logger.debug(
                f"Added confidence indicators for business {merged_business.business_id}"
            )
    ...
    def _calculate_location_confidence(self, location: BusinessLocation) -> float:
        ...
        address_completeness = sum(
            1 for field in address_fields if field and field.strip()
        ) / len(address_fields)
    ...
    def _calculate_contact_confidence(
        self, contact_info: Optional[BusinessContactInfo]
    ) -> float:
        """Calculate confidence score for contact information."""
        if not contact_info:
            return 0.0

        contact_fields = [contact_info.phone, contact_info.website, contact_info.email]

        # Count non-empty fields
        sum(1 for field in contact_fields if field and field.strip())

        # Basic validation for filled fields
        phone_valid = (
            contact_info.phone and contact_info.phone.strip()
        ) and (
            contact_info.phone.startswith("+")
            or contact_info.phone.replace("-", "")
            .replace("(", "")
            .replace(")", "")
            .replace(" ", "")
            .isdigit()
        ...
        email_valid = (
            contact_info.email and contact_info.email.strip()
        ) and (
            "@" in contact_info.email and "." in contact_info.email.split("@")[1]
        )

        website_valid = (
            contact_info.website and contact_info.website.strip()
        ) and (
            contact_info.website.startswith(("http://", "https://", "www."))
        )
        if website_valid:
            # Check if there's a domain after the protocol
            if contact_info.website.startswith("http://") and len(contact_info.website) > 7:
                domain_part = contact_info.website[7:]
                if "." in domain_part and len(domain_part.split(".")[0]) > 0:
                    website_valid = True
            elif contact_info.website.startswith("https://") and len(contact_info.website) > 8:
                domain_part = contact_info.website[8:]
                if "." in domain_part and len(domain_part.split(".")[0]) > 0:
                    website_valid = True
            elif contact_info.website.startswith("www.") and len(contact_info.website) > 4:
                domain_part = contact_info.website[4:]
                if "." in domain_part and len(domain_part.split(".")[0]) > 0:
                    website_valid = True
            else:
                website_valid = False

        return (phone_valid + email_valid + website_valid) / 3.0
    ...
    def _calculate_categories_confidence(
    ...
    def _calculate_source_confidence(
        self, completeness_scores: List[DataCompletenessScore]
    ) -> float:
    ...
    def _calculate_conflict_resolution_confidence(
    ...
    def _calculate_data_consistency_confidence(
        self, completeness_scores: List[DataCompletenessScore]
    ) -> float:
