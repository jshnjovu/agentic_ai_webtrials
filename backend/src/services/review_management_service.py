...
class ReviewManagementService(BaseService):
    ...
    def __init__(self):
        ...
        self.UNCERTAIN_MATCH_THRESHOLD = 0.5  # Lowered to catch medium confidence + conflicts
    ...
    def flag_uncertain_matches(self, 
                              merged_business: MergedBusinessData,
                              conflicts_resolved: List[MergeConflict],
                              source_data: List[BusinessSourceData]) -> Optional[ReviewFlag]:
        """
        ...
        try:
            ...
            self.log_operation("flag_uncertain_matches", getattr(merged_business, 'run_id', None))
            
            # Check if flagging is needed
            should_flag = self._should_flag_for_review(merged_business, conflicts_resolved)
            
            if not should_flag:
                self.logger.debug(f"No flagging needed for business {merged_business.business_id}")
                return None
            ...
            flag = ReviewFlag(
                ...
                flag_type=self._determine_flag_type(merged_business, conflicts_resolved),
            ...
            self.logger.info(f"Created review flag {flag.flag_id} for business {merged_business.business_id}")
            return flag
    ...
    def create_review_workflow(self, review_flag: ReviewFlag) -> List[ReviewWorkflow]:
        ...
        try:
            ...
            self.logger.info(f"Created review workflow with {len(workflow_steps)} steps for flag {review_flag.flag_id}")
    ...
    def assign_review(self, review_flag: ReviewFlag, user_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Assigned review flag {review_flag.flag_id} to user {user_id}")
            return True
    ...
    def update_review_status(self, 
        ...
        try:
            ...
            self.logger.info(f"Updated review flag {review_flag.flag_id} status to {new_status}")
            return True
    ...
    def get_pending_reviews(self, 
                           ...
                           assigned_to: Optional[str] = None) -> List[ReviewFlag]:
        """
    ...
    def resolve_review_flag(self, 
        ...
        try:
            ...
            self.logger.info(f"Resolved review flag {review_flag.flag_id} with action: {resolution_action}")
            return True
    ...
    def _should_flag_for_review(self, 
                               merged_business: MergedBusinessData,
                               conflicts_resolved: List[MergeConflict]) -> bool:
        ...
        if conflicts_resolved:
            ...
            if merged_business.confidence_level == ConfidenceLevel.MEDIUM:
                ...
                low_confidence_conflicts = [c for c in conflicts_resolved if c.confidence < 0.7]
                return len(low_confidence_conflicts) > 0
            
            # For high confidence businesses, use scoring
            review_score = self._calculate_review_score(merged_business, conflicts_resolved)
    ...
    def _determine_flag_type(self, 
                            merged_business: MergedBusinessData,
                            conflicts_resolved: List[MergeConflict]) -> str:
    ...
    def _determine_priority(self, 
                           merged_business: MergedBusinessData,
                           conflicts_resolved: List[MergeConflict]) -> ReviewPriority:
        ...
        if conflicts_resolved:
            ...
            if merged_business.confidence_level == ConfidenceLevel.MEDIUM:
                ...
                if len(low_confidence_conflicts) >= 2:
                    ...
                    return ReviewPriority.LOW  # High confidence conflicts don't need high priority
            
            priority_score = self._calculate_priority_score(merged_business, conflicts_resolved)
    ...
    def _generate_flag_reason(self, 
                             merged_business: MergedBusinessData,
                             conflicts_resolved: List[MergeConflict]) -> str:
        ...
        if conflicts_resolved:
            ...
            avg_confidence = sum(c.confidence for c in conflicts_resolved) / len(conflicts_resolved)
            ...
            critical_conflicts = [c for c in conflicts_resolved if c.field_name in critical_fields]
        ...
        if conflicts_resolved and merged_business.confidence_level != ConfidenceLevel.LOW:
            review_score = self._calculate_review_score(merged_business, conflicts_resolved)
            if review_score >= 0.8:
    ...
    def _calculate_review_score(self, 
                               merged_business: MergedBusinessData,
        ...
        hybrid_score = (base_score * weights["confidence"] + 
    ...
    def _calculate_priority_score(self, 
        ...
        confidence_factor = 1.0 - self._confidence_level_to_score(merged_business.confidence_level)
        ...
        priority_score = (confidence_factor * weights["confidence"] + 
    ...
    def _calculate_conflict_complexity_score(self, conflicts_resolved: List[MergeConflict]) -> float:
        """
        ...
        count_factor = min(len(conflicts_resolved) / 3.0, 1.0)  # Cap at 3 conflicts, increased sensitivity
        
        # Confidence factor: lower confidence conflicts = higher complexity (increased sensitivity)
        avg_confidence = sum(c.confidence for c in conflicts_resolved) / len(conflicts_resolved)
        confidence_factor = 1.0 - avg_confidence  # Invert so lower confidence = higher complexity
        ...
        for conflict in conflicts_resolved:
            ...
            strategy_weight = self._get_resolution_strategy_weight(conflict.resolution_strategy)
            
            field_complexity = (1.0 - conflict.confidence) * field_weight * strategy_weight
        ...
        complexity_score = (count_factor * weights["count"] + 
    ...
    def _calculate_conflict_severity_score(self, conflicts_resolved: List[MergeConflict]) -> float:
        """
        ...
        count_severity = min(len(conflicts_resolved) / 2.0, 1.0)  # Cap at 2 conflicts for severity
        ...
        critical_conflicts = [c for c in conflicts_resolved if c.field_name in critical_fields]
        criticality_factor = len(critical_conflicts) / len(conflicts_resolved) if conflicts_resolved else 0.0
        ...
        severity_score = (count_severity * weights["count"] + 
