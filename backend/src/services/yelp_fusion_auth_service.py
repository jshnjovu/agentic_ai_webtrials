...
class YelpFusionAuthService(BaseService):
    ...
    def authenticate(self, run_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check rate limiting
            can_request, reason = self.rate_limiter.can_make_request("yelp_fusion", run_id)
            if not can_request:
            ...
            with httpx.Client(timeout=self.api_config.API_TIMEOUT_SECONDS) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "businesses" in result:
                        self.log_operation("Yelp Fusion API authentication successful", run_id=run_id)
                        return {
                            "success": True,
                        ...
                        return self.handle_error(
                            Exception("API response missing expected 'businesses' field"),
    ...
    def test_connection(self, run_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            with httpx.Client(timeout=self.api_config.API_TIMEOUT_SECONDS) as client:
                ...
                if response.status_code == 200:
                    self.log_operation("Yelp Fusion API connection test successful", run_id=run_id)
                    return {
                        "success": True,
                        "api_name": "yelp_fusion",
