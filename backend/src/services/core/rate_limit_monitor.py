...
class RateLimitMonitor(BaseService):
    ...
    def _check_api_rate_limit(self, api_name: str, run_id: Optional[str]) -> Dict[str, Any]:
        """
    ...
    def _generate_alerts(
        ...
        if usage_percentage >= thresholds.get("warning", 0.7):
            alert = RateLimitAlert(
                ...
                reset_time=self.rate_limiter.get_rate_limit_info(api_name)["reset_time"],
                run_id=run_id
            )
            alerts.append(alert)
    ...
    def _store_alerts(self, new_alerts: List[RateLimitAlert]):
        ...
        for alert in new_alerts:
            if alert.level == AlertLevel.CRITICAL:
                self.logger.error(f"CRITICAL ALERT: {alert.message} for {alert.api_name}")
    ...
    def _cleanup_old_alerts(self):
        ...
        if len(self.alerts) > self.max_alerts_per_api * len(self.alert_thresholds):
            ...
            self.alerts = self.alerts[:self.max_alerts_per_api * len(self.alert_thresholds)]
            ...
            for api_name in self.alert_thresholds:
                ...
                if rate_limit_info:
                    summary["apis"][api_name] = {
                        "current_usage": rate_limit_info["current_usage"],
                        "limit": rate_limit_info["limit"],
                        "usage_percentage": rate_limit_info["current_usage"] / rate_limit_info["limit"],
                        "remaining": rate_limit_info["remaining"],
                        "reset_time": rate_limit_info["reset_time"]
                    }
