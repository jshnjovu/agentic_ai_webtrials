...
class GooglePlacesService(BaseService):
    ...
    def search_businesses(self, request: BusinessSearchRequest) -> BusinessSearchResponse | BusinessSearchError:
        ...
        try:
            ...
            # Check rate limiting
            can_request, reason = self.rate_limiter.can_make_request("google_places", request.run_id)
            if not can_request:
    ...
    def _process_location(self, location: str, location_type: LocationType) -> Dict[str, Any]:
        """
        ...
        try:
            if location_type == LocationType.COORDINATES:
                ...
                return {"valid": True, "text": location.strip(), "type": location_type.value}
    ...
    def _process_business_results(self, raw_results: List[Dict[str, Any]], max_results: int, run_id: Optional[str]) -> List[BusinessData]:
        """
    ...
    def get_next_page(self, next_page_token: str, run_id: Optional[str] = None) -> Dict[str, Any]:
        """
