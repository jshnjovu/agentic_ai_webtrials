...
class ReviewFlag(BaseModel):
    ...
    flag_type: str = Field(..., description="Type of flag (e.g., 'uncertain_match', 'data_conflict')")
    ...
    confidence_score: float = Field(..., description="Confidence score that triggered the flag", ge=0.0, le=1.0)
    source_data: List[BusinessSourceData] = Field(..., description="Source data that caused the flag")
    conflicts: List[MergeConflict] = Field(default_factory=list, description="Conflicts that need resolution")
...
class ReviewWorkflow(BaseModel):
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="Step creation timestamp")
...
class ReviewWorkflowResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether the operation was successful")
    workflow_steps: List[ReviewWorkflow] = Field(..., description="List of workflow steps")
    message: str = Field(..., description="Response message")
    run_id: Optional[str] = Field(None, description="Processing run identifier")
...
class PendingReviewsResponse(BaseModel):
    ...
    pending_reviews: List[ReviewFlag] = Field(..., description="List of pending review flags")
