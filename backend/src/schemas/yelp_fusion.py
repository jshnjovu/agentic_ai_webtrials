...
class YelpBusinessSearchRequest(BaseModel):
    ...
    location_type: YelpLocationType = Field(
        default=YelpLocationType.CITY, description="Type of location input"
    )
    categories: Optional[List[str]] = Field(
        None, description="List of business categories to filter by"
    )
...
class YelpBusinessHours(BaseModel):
    ...
    is_overnight: bool = Field(
        default=False, description="Whether business operates overnight"
    )
...
class YelpBusinessLocation(BaseModel):
    ...
    display_address: List[str] = Field(
        default_factory=list, description="Formatted display address"
    )
...
class YelpBusinessData(BaseModel):
    ...
    categories: List[YelpBusinessCategory] = Field(
        default_factory=list, description="Business categories"
    )
    rating: float = Field(
        ..., description="Business rating (0.0 to 5.0)", ge=0.0, le=5.0
    )
    coordinates: YelpBusinessCoordinates = Field(
        ..., description="Business coordinates"
    )
    transactions: List[str] = Field(
        default_factory=list, description="Available transaction types"
    )
    price: Optional[str] = Field(None, description="Price level indicator (1-4)")
    location: YelpBusinessLocation = Field(
        ..., description="Business location information"
    )
    phone: Optional[str] = Field(None, description="Business phone number")
    display_phone: Optional[str] = Field(
        None, description="Formatted display phone number"
    )
    distance: Optional[float] = Field(
        None, description="Distance from search location in meters"
    )
    hours: Optional[List[YelpBusinessHours]] = Field(
        None, description="Business operating hours"
    )
    photos: Optional[List[str]] = Field(None, description="Business photo URLs")
    # Enhanced data fields
    attributes: Optional[Dict[str, Any]] = Field(
        None, description="Business attributes (accessibility, amenities, etc.)"
    )
    special_hours: Optional[List[Dict[str, Any]]] = Field(
        None, description="Special hours for holidays or events"
    )
    business_status: Optional[str] = Field(
        None, description="Current business status or announcements"
    )
    social_media: Optional[Dict[str, str]] = Field(
        None, description="Social media links if available"
    )
...
class YelpBusinessSearchResponse(BaseModel):
    ...
    businesses: List[YelpBusinessData] = Field(
        ..., description="List of business results"
    )
    region: Optional[Dict[str, Any]] = Field(
        None, description="Search region information"
    )
    run_id: Optional[str] = Field(
        None, description="Unique identifier for the processing run"
    )
...
class YelpPaginationInfo(BaseModel):
    ...
    previous_offset: Optional[int] = Field(
        None, description="Previous offset for pagination"
    )
