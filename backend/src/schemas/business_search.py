...
class BusinessData(BaseModel):
    ...
    rating: Optional[float] = Field(None, description="Business rating (0.0 to 5.0)", ge=0.0, le=5.0)
    user_ratings_total: Optional[int] = Field(None, description="Total number of user ratings", ge=0)
    price_level: Optional[int] = Field(None, description="Price level indicator (0-4)", ge=0, le=4)
    types: Optional[List[str]] = Field(None, description="List of business types/categories")
    geometry: Optional[Dict[str, Any]] = Field(None, description="Location coordinates and viewport")
...
class PaginationInfo(BaseModel):
    ...
    previous_page_token: Optional[str] = Field(None, description="Token for previous page")
