...
class BusinessSourceData(BaseModel):
    ...
    source: str = Field(..., description="Data source (e.g., 'google_places', 'yelp_fusion')")
    ...
    contact_info: Optional[BusinessContactInfo] = Field(None, description="Contact information")
    ...
    price_level: Optional[int] = Field(None, description="Price level indicator", ge=0, le=4)
...
class BusinessMatchScore(BaseModel):
    ...
    name_similarity: float = Field(..., description="Name similarity score (0.0 to 1.0)", ge=0.0, le=1.0)
    address_similarity: float = Field(..., description="Address similarity score (0.0 to 1.0)", ge=0.0, le=1.0)
    coordinate_proximity: float = Field(..., description="Coordinate proximity score (0.0 to 1.0)", ge=0.0, le=1.0)
...
class BusinessMatchCandidate(BaseModel):
    ...
    source_data: BusinessSourceData = Field(..., description="Business data from source")
    ...
    needs_review: bool = Field(default=False, description="Whether this needs manual review")


class BusinessMatchingRequest(BaseModel):
    """Request for business matching."""
    
    businesses: List[BusinessSourceData] = Field(..., description="List of businesses to match", min_length=2)
    name_weight: float = Field(default=0.4, description="Weight for name similarity (0.0 to 1.0)", ge=0.0, le=1.0)
    address_weight: float = Field(default=0.3, description="Weight for address similarity (0.0 to 1.0)", ge=0.0, le=1.0)
    coordinate_weight: float = Field(default=0.3, description="Weight for coordinate proximity (0.0 to 1.0)", ge=0.0, le=1.0)
...
class BusinessMatchingResponse(BaseModel):
    ...
    matched_groups: List[List[BusinessMatchCandidate]] = Field(..., description="Groups of matched businesses")
    unmatched_businesses: List[BusinessSourceData] = Field(..., description="Businesses that couldn't be matched")
    matching_metadata: Dict[str, Any] = Field(..., description="Matching process metadata")
    run_id: Optional[str] = Field(None, description="Unique identifier for the processing run")
