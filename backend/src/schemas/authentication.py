...
class AuthenticationResponse(BaseModel):
    ...
    message: str = Field(..., description="Human-readable message about the authentication result")
    run_id: Optional[str] = Field(None, description="Unique identifier for the processing run")
    error: Optional[str] = Field(None, description="Error message if authentication failed")
    details: Optional[Dict[str, Any]] = Field(None, description="Additional authentication details")
...
class HealthCheckResponse(BaseModel):
    ...
    apis: Dict[str, Dict[str, Any]] = Field(..., description="Status of individual APIs")
    run_id: Optional[str] = Field(None, description="Unique identifier for the processing run")
