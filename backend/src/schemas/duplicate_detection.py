...
class BusinessFingerprint(BaseModel):
    ...
    coordinate_hash: str = Field(
        ..., description="Hash of coordinates for proximity matching"
    )
...
class DuplicateGroup(BaseModel):
    ...
    primary_business: BusinessSourceData = Field(
        ..., description="Primary business record"
    )
    duplicate_businesses: List[BusinessSourceData] = Field(
        ..., description="List of duplicate businesses"
    )
    duplicate_type: DuplicateType = Field(
        ..., description="Type of duplication detected"
    )
    confidence_score: float = Field(
        ...,
        description="Confidence in duplicate detection (0.0 to 1.0)",
        ge=0.0,
        le=1.0,
    )
    ...
    detection_metadata: Dict[str, Any] = Field(
        ..., description="Metadata about detection process"
    )
    created_at: str = Field(..., description="Group creation timestamp")
    needs_review: bool = Field(
        default=False, description="Whether this group needs manual review"
    )
...
class DuplicateDetectionRequest(BaseModel):
    ...
    businesses: List[BusinessSourceData] = Field(
        ..., description="List of businesses to check for duplicates", min_length=1
    )
    detection_threshold: float = Field(
        default=0.8, description="Threshold for duplicate detection", ge=0.0, le=1.0
    )
    auto_remove_high_confidence: bool = Field(
        default=True, description="Automatically remove high-confidence duplicates"
    )
    include_fingerprints: bool = Field(
        default=True, description="Include business fingerprints in response"
    )
...
class DuplicateDetectionResponse(BaseModel):
    ...
    duplicate_groups: List[DuplicateGroup] = Field(
        ..., description="Groups of duplicate businesses found"
    )
    unique_businesses: List[BusinessSourceData] = Field(
        ..., description="Businesses with no duplicates"
    )
    removed_duplicates: List[BusinessSourceData] = Field(
        ..., description="Duplicates that were automatically removed"
    )
    detection_metadata: Dict[str, Any] = Field(
        ..., description="Metadata about detection process"
    )
...
class DuplicateRemovalRequest(BaseModel):
    ...
    duplicate_groups: List[DuplicateGroup] = Field(
        ..., description="Duplicate groups to process"
    )
    removal_strategy: str = Field(
        default="keep_primary", description="Strategy for removing duplicates"
    )
    review_threshold: float = Field(
        default=0.7,
        description="Threshold below which manual review is required",
        ge=0.0,
        le=1.0,
    )
    run_id: Optional[str] = Field(
...
class DuplicateRemovalResponse(BaseModel):
    ...
    total_groups_processed: int = Field(
        ..., description="Total number of duplicate groups processed", ge=0
    )
    duplicates_removed: List[BusinessSourceData] = Field(
        ..., description="Duplicates that were removed"
    )
    businesses_kept: List[BusinessSourceData] = Field(
        ..., description="Businesses that were kept"
    )
    review_required: List[DuplicateGroup] = Field(
        ..., description="Groups requiring manual review"
    )
    removal_metadata: Dict[str, Any] = Field(
        ..., description="Metadata about removal process"
    )
    run_id: Optional[str] = Field(
        None, description="Unique identifier for the processing run"
    )
