...
class DataCompletenessScore(BaseModel):
    ...
    overall_score: float = Field(..., description="Overall completeness score (0.0 to 1.0)", ge=0.0, le=1.0)
    name_score: float = Field(..., description="Name completeness score (0.0 to 1.0)", ge=0.0, le=1.0)
    location_score: float = Field(..., description="Location completeness score (0.0 to 1.0)", ge=0.0, le=1.0)
    contact_score: float = Field(..., description="Contact information completeness score (0.0 to 1.0)", ge=0.0, le=1.0)
    rating_score: float = Field(..., description="Rating and review completeness score (0.0 to 1.0)", ge=0.0, le=1.0)
...
class MergedBusinessData(BaseModel):
    ...
    business_id: str = Field(..., description="Unique identifier for the merged business")
    name: str = Field(..., description="Primary business name")
    location: BusinessLocation = Field(..., description="Primary business location")
    contact_info: Optional[BusinessContactInfo] = Field(None, description="Merged contact information")
    ...
    categories: List[str] = Field(default_factory=list, description="Combined categories from all sources")
    price_level: Optional[int] = Field(None, description="Primary price level", ge=0, le=4)
    hours: Optional[Dict[str, Any]] = Field(None, description="Primary business hours")
    photos: List[Dict[str, Any]] = Field(default_factory=list, description="Combined photos from all sources")
    confidence_level: ConfidenceLevel = Field(..., description="Overall confidence in merged data")
    source_contributions: List[str] = Field(..., description="List of sources that contributed data")
    merge_metadata: Dict[str, Any] = Field(..., description="Metadata about the merging process")
...
class MergeConflict(BaseModel):
    ...
    source_values: Dict[str, Any] = Field(..., description="Values from different sources")
    resolution_strategy: str = Field(..., description="Strategy used to resolve the conflict")
...
class BusinessMergeRequest(BaseModel):
    ...
    businesses: List[BusinessSourceData] = Field(..., description="List of businesses to merge", min_length=2)
    merge_strategy: str = Field(default="completeness", description="Strategy for merging conflicting data")
    prioritize_source: Optional[str] = Field(None, description="Source to prioritize when conflicts exist")
...
class BusinessMergeResponse(BaseModel):
    ...
    merged_business: MergedBusinessData = Field(..., description="Merged business record")
    conflicts_resolved: List[MergeConflict] = Field(..., description="List of conflicts that were resolved")
    merge_metadata: Dict[str, Any] = Field(..., description="Metadata about the merging process")
    run_id: Optional[str] = Field(None, description="Unique identifier for the processing run")
