...
async def run_lighthouse_audit(
    ...
    try:
        ...
        if not service.validate_input(request.dict()):
            raise HTTPException(
                status_code=400, detail="Invalid Lighthouse audit request"
            )
        ...
        response = LighthouseAuditResponse(
            ...
            scores={
                ...
                "accessibility": audit_result.get("scores", {}).get(
                    "accessibility", 0.0
                ),
                "best_practices": audit_result.get("scores", {}).get(
                    "best_practices", 0.0
                ),
        ...
        background_tasks.add_task(
            _persist_audit_results, audit_result, request.business_id, request.run_id
        )
...
async def run_heuristic_evaluation(
    ...
    try:
        ...
        if not service.validate_input(request.dict()):
            raise HTTPException(
                status_code=400, detail="Invalid heuristic evaluation request"
            )
        ...
        response = HeuristicEvaluationResponse(
            ...
            business_id=request.business_id,
            run_id=request.run_id,
            evaluation_timestamp=evaluation_result.get(
                "evaluation_timestamp", time.time()
            ),
...
async def run_fallback_scoring(
    ...
    try:
        ...
        if not service.validate_input(request.dict()):
            raise HTTPException(
                status_code=400, detail="Invalid fallback scoring request"
            )
        ...
        if not fallback_result.get("success", False):
            ...
            if fallback_result.get("error_code") == "RATE_LIMIT_EXCEEDED":
                ...
                raise HTTPException(
                    status_code=400,
                    detail={
                        "error": fallback_result.get(
                            "error", "Fallback scoring failed"
                        ),
                        "error_code": fallback_result.get("error_code"),
                        "context": fallback_result.get("context", "fallback_execution"),
                        "website_url": str(request.website_url),
...
async def validate_website_scores(
    ...
    score_validation_service: ScoreValidationService = Depends(
...
async def website_scoring_health_check(
    ...
    heuristic_service: HeuristicEvaluationService = Depends(
        get_heuristic_evaluation_service
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Website scoring services unhealthy: {str(e)}"
        )


async def _persist_audit_results(
    audit_result: dict, business_id: str, run_id: str
) -> None:
    """
...
async def _persist_heuristic_results(
    evaluation_result: dict, business_id: str, run_id: str
) -> None:
    """
    ...
    try:
        ...
        # For now, just log the persistence attempt
        print(
            f"Would persist heuristic evaluation results for business {business_id}, run {run_id}"
        )
...
async def _persist_fallback_results(
    fallback_result: dict, business_id: str, run_id: str
) -> None:
    """
    ...
    try:
        ...
        # For now, just log the persistence attempt
        print(
            f"Would persist fallback results for business {business_id}, run {run_id}"
        )
...
async def _persist_validation_results(
    validation_result: dict, business_id: str, run_id: str
) -> None:
    """
    ...
    try:
        ...
        # For now, just log the persistence attempt
        print(
            f"Would persist validation results for business {business_id}, run {run_id}"
        )
