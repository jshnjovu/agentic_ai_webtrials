...
class ChatResponse(BaseModel):
    ...
    workflow_progress: Optional[Dict[str, Any]] = Field(
        None, description="Current workflow progress"
    )
...
async def chat_with_ai(
    chat_message: ChatMessage, background_tasks: BackgroundTasks
) -> ChatResponse:
    """
    ...
    try:
        ...
        context_manager.get_or_create_workflow_state(session_id)
...
async def get_active_sessions() -> List[SessionInfo]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve active sessions: {str(e)}"
        )
...
async def get_session_progress(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve session progress: {str(e)}"
        )


@router.post("/sessions/{session_id}/confirm")
async def confirm_action(session_id: str, confirmed: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if result is None:
            raise HTTPException(
                status_code=400, detail="No pending confirmation for this session"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to resolve confirmation: {str(e)}"
        )
