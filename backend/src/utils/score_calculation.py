...
def normalize_score(
    score: float,
    from_scale: Tuple[float, float] = (0, 1),
    to_scale: Tuple[float, float] = (0, 100),
) -> float:
    """
...
def calculate_performance_trend(scores: List[float]) -> Dict[str, float]:
    ...
    """
    if len(scores) < 2:
        return {"trend": 0.0, "improvement_rate": 0.0, "volatility": 0.0}
...
def get_score_insights(scores: Dict[str, float]) -> List[str]:
    ...
    if scores.get("performance", 0) < 50:
        insights.append(
            "Performance score is critically low - immediate optimization needed"
        )
    ...
    if scores.get("accessibility", 0) < 80:
        insights.append(
            "Accessibility score below recommended threshold - review WCAG compliance"
        )
    ...
    if overall >= 90:
        ...
        insights.append(
            "Acceptable performance - consider improvements for better user experience"
        )
    else:
        insights.append(
            "Performance needs significant improvement - prioritize optimization"
        )
