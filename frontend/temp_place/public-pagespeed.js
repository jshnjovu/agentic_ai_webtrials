...
class PublicPageSpeedInsightsAPI {
  constructor() {
    this.baseUrl = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed';
  }

  // Run PageSpeed Insights analysis using public API
  async runPageSpeedAnalysis(url, strategy = 'mobile') {
    try {
      console.log(`\n🔍 Analyzing: ${url} (${strategy})`);
      
      const apiUrl = `${this.baseUrl}?url=${encodeURIComponent(url)}&strategy=${strategy}&category=performance&category=accessibility&category=best-practices&category=seo&prettyPrint=true`;
...
async function main() {
  const api = new PublicPageSpeedInsightsAPI();
  
  try {
    console.log('🚀 Starting Public PageSpeed Insights API test...\n');
    ...
    for (const url of testUrls) {
      try {
        ...
        const filename = `public-pagespeed-results-${new Date().toISOString().split('T')[0]}.json`;
        const allResults = {
...
module.exports = PublicPageSpeedInsightsAPI;
