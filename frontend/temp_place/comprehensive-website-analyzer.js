...
class ComprehensiveWebsiteAnalyzer {
  ...
  async runPageSpeedAnalysis(url) {
    try {
      // Use public PageSpeed Insights API
      const apiUrl = `${this.pagespeedBaseUrl}?url=${encodeURIComponent(url)}&strategy=mobile&category=performance&category=accessibility&category=best-practices&category=seo&prettyPrint=true`;
